...
namespace Persistencia.DAL.EntityFramework
{
    public class RepositorioCliente : Repositorio<Cliente, HotelContext>, IRepositorioCliente
    {
        ...
        public IEnumerable<Cliente> ObtenerClientesPorNomyAp(string pNombre, bool pValor)
        {
            var clientes = from cli in this.iDbContext.Clientes.Include("TarifaCliente").Include("Domicilio.Ciudad")
                           where ((cli.Nombre+cli.Apellido).Contains(pNombre))&&(cli.EnAlta == pValor)
                           select cli;

            return clientes.ToList<Cliente>();
        }

        public Cliente GetPorDNI(int pId, bool pValor)
        {
            try
            {
                return iDbContext.Clientes.Include("TarifaCliente").Include("Domicilio.Ciudad").Where(c => c.ClienteId == pId && c.EnAlta == pValor).Single();
            }
            catch
            {
        ...
        public Cliente GetPorLegajo(int pLegajo, bool pValor)
        {
            try
            {
                return iDbContext.Clientes.Include("TarifaCliente").Include("Domicilio.Ciudad").Where(c => c.Legajo == pLegajo && c.EnAlta == pValor).Single();
            }
            catch
            {
                throw new Exception("Cliente No Existe");
